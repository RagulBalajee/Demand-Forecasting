    ...
    if st.button("🚀 Generate Forecast", type="primary"):
        with st.spinner("Training models and generating forecast..."):
            try:
                ...
                st.info(f"📊 Found {len(filtered_df)} records for the selected filters.")
                if len(filtered_df) < 10:
                    st.error("❌ Insufficient data for forecasting. Please select different filters.")
                    return

                # --- NEW ROBUST DATA PIPELINE ---
                
                # 1. Create a future dataframe shell
                future_dates = pd.date_range(start=forecast_start, periods=forecast_days, freq='D')
                future_df_shell = pd.DataFrame({'Date': future_dates})
                
                # Get identifiers (Store/Product) for the future predictions
                group_cols = [col for col in ['Store', 'Product'] if col in filtered_df.columns]
                if group_cols:
                    identifiers = filtered_df[group_cols].iloc[0].to_dict()
                    for col, val in identifiers.items():
                        future_df_shell[col] = val

                # 2. Combine historical and future data
                # The 'Sales' column will correctly have NaNs for future dates
                combined_df = pd.concat([filtered_df, future_df_shell], ignore_index=True)

                # 3. Engineer features ONCE on the combined data
                feature_engineer = components['feature_engineer']
                st.info("⚙️ Engineering features for training and prediction...")
                
                # The feature engineering logic needs to be robust for this
                # Let's use a simplified but safe approach here directly
                engineered_df = combined_df.copy()
                engineered_df['DayOfWeek'] = engineered_df['Date'].dt.dayofweek
                engineered_df['Month'] = engineered_df['Date'].dt.month
                engineered_df['IsWeekend'] = (engineered_df['DayOfWeek'] >= 5).astype(int)
                
                # Create lag/rolling features on the combined df
                # This correctly uses historical sales for future feature calculation
                if 'Sales' in engineered_df.columns:
                    engineered_df['Sales_Lag7'] = engineered_df.groupby(group_cols)['Sales'].shift(7)
                    engineered_df['Sales_MA14'] = engineered_df.groupby(group_cols)['Sales'].rolling(14, min_periods=1).mean().reset_index(level=group_cols, drop=True)
                    # Backfill to handle NaNs at the start of the future period
                    engineered_df.fillna(method='bfill', inplace=True)
                    engineered_df.fillna(method='ffill', inplace=True) # Fill any remaining NaNs
                
                # 4. Split back into training and prediction sets
                # Convert forecast_start to a timestamp for robust comparison
                forecast_start_ts = pd.to_datetime(forecast_start)
                train_engineered = engineered_df[engineered_df['Date'] < forecast_start_ts]
                future_engineered = engineered_df[engineered_df['Date'] >= forecast_start_ts]

                # 5. Train the model
                st.info(f"🤖 Training model on {len(train_engineered)} samples...")
                forecaster = components['forecaster']
                X_train, y_train = forecaster.prepare_data(train_engineered)
                
                if len(X_train) == 0:
                    st.error("❌ No training samples available after feature engineering. Please check data.")
                    return
                
                model_name = "random_forest" # Keep it simple for now
                forecaster.train_models(X_train, y_train, [model_name])

                # 6. Make Predictions
                st.info(f"🔮 Predicting for {len(future_engineered)} future dates...")
                # Ensure future dataframe has the same columns as training features
                X_future = future_engineered[forecaster.feature_columns]
                
                predictions = forecaster.predict(X_future, model_name=model_name)
                
                # Create final forecast dataframe
                forecast_df = future_engineered[['Date'] + group_cols].copy()
                forecast_df['Sales'] = predictions

                # Store results for display
                st.session_state.forecast_results = {
                    'historical': filtered_df,
                    'forecast': forecast_df,
                    'performance': forecaster.train_models(X_train, y_train, [model_name])[model_name], # Re-run for metrics
...
def sales_calendar_page():
    ...
    # Create a proper grid for the heatmap
    calendar_grid = []
    day_names = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
    
    # Add header row
    calendar_grid.append(day_names)
    ...
    for week in month_calendar:
        ...
        for day in week:
            if day == 0:  # Empty day
                ...
                week_data.append(day_sales)
        calendar_grid.append(week_data)
    
    # Convert to numpy array for plotting
    calendar_array = np.array(calendar_grid[1:])  # Skip header row
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title="Week of Month",
            tickmode='array',
            tickvals=list(range(len(calendar_array))),
            ticktext=[f"Week {i+1}" for i in range(len(calendar_array))]
        )
    )
...
def main():
    ...
    # Auto-load Rossmann data if available at startup
    if not st.session_state.data_loaded and os.path.exists('data/rossmann-store-sales'):
        try:
            ...
            st.toast(f"Auto-load failed: {e}", icon="❌")

    # --- Top Navigation using Buttons ---
    page_options = {
        "🏠 Home": home_page,
    ...
    for i, (page, func) in enumerate(page_options.items()):
    ...
    if st.session_state.data_loaded:
        st.info(f"✅ Using dataset: **{st.session_state.dataset_name}** with **{len(st.session_state.sales_data):,}** records loaded.")
    
    # --- Render the selected page ---
    page_func = page_options.get(st.session_state.page)
    if page_func:
        page_func()
