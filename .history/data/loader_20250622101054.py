...
class DataLoader:
    ...
    def preprocess_data(self, sales_df: pd.DataFrame, store_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        ...
        # Rolling averages
        df['Sales_MA7'] = df.groupby(['Store', 'Product'])['Sales'].rolling(7).mean().reset_index(0, drop=True)
        df['Sales_MA30'] = df.groupby(['Store', 'Product'])['Sales'].rolling(30).mean().reset_index(0, drop=True)
