...
class FeatureEngineer:
    ...
    def create_rolling_features(self, df: pd.DataFrame, target_col: str = 'Sales',
        ...
        for window in windows:
            ...
            df[mean_col] = df.groupby(group_cols)[target_col].rolling(window).mean().reset_index(0, drop=True)
            
            # Rolling std
            std_col = f'{target_col}_STD{window}'
            df[std_col] = df.groupby(group_cols)[target_col].rolling(window).std().reset_index(0, drop=True)
            
            # Rolling min/max
            min_col = f'{target_col}_Min{window}'
            df[min_col] = df.groupby(group_cols)[target_col].rolling(window).min().reset_index(0, drop=True)
            
            max_col = f'{target_col}_Max{window}'
            df[max_col] = df.groupby(group_cols)[target_col].rolling(window).max().reset_index(0, drop=True)
