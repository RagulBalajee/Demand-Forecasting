...
class FeatureEngineer:
    ...
    def create_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Fix: Use encoded or numeric StateHoliday for interaction
        state_holiday_col = None
        if 'StateHoliday_Encoded' in df.columns:
            state_holiday_col = 'StateHoliday_Encoded'
        elif 'StateHoliday' in df.columns:
            if not np.issubdtype(df['StateHoliday'].dtype, np.number):
                df['StateHoliday_tmp'] = pd.Categorical(df['StateHoliday']).codes
                state_holiday_col = 'StateHoliday_tmp'
            else:
                state_holiday_col = 'StateHoliday'
        if state_holiday_col and 'Sales_Lag7' in df.columns:
            df['StateHoliday_Sales_Lag7'] = df[state_holiday_col] * df['Sales_Lag7']
            if state_holiday_col == 'StateHoliday_tmp':
                df.drop(columns=['StateHoliday_tmp'], inplace=True)
