    ...
    if st.button("🚀 Generate Forecast", type="primary"):
        with st.spinner("Training models and generating forecast..."):
            try:
                ...
                # Create a stable dataframe for training and prediction context
                train_df = filtered_df.copy()
                
                # Check if we have enough data
                if len(train_df) < 10:
                    st.error(f"❌ Insufficient data for forecasting. Only {len(train_df)} records found.")
                ...
                if len(train_df) < 50:
                    st.info(f"📊 Using simplified feature engineering for small dataset.")
                    
                    if 'Sales' not in train_df.columns:
                        raise ValueError("Critical error: 'Sales' column is missing before feature engineering.")
                    ...
                    enhanced_df['Sales_Lag1'] = enhanced_df['Sales'].shift(1).fillna(method='bfill').fillna(0)
                    ...
                    engineered_df = enhanced_df
                else:
                    # Use full feature engineering for larger datasets
                    engineered_df = feature_engineer.engineer_all_features(train_df)
                
                # Train model
                forecaster = components['forecaster']
                X, y = forecaster.prepare_data(engineered_df)
                ...
                if model_type != "Prophet":
                    ...
                    if len(X) < 50:
                        ...
                        module_name, class_name = model_config.get(model_name, ("sklearn.ensemble", "GradientBoostingRegressor"))
                        
                        import importlib
                        module = importlib.import_module(module_name)
                        ModelClass = getattr(module, class_name)
                        model = ModelClass(n_estimators=50, random_state=42)
                    ...
                    # Make predictions using the original filtered data as context
                    forecast_df = forecaster.predict_future(train_df, future_dates, feature_engineer, model_name)
                    
                    # Store results
                    st.session_state.forecast_results = {
                        'historical': train_df, # Use the original filtered data for charting
